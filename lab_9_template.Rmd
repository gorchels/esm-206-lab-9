...
by_sqft <- ggplot(homes_sub, aes(x = SqFt, y = Price)) +
  geom_point(aes(color = City, pch = Status), alpha = 0.5) + #alpha makes it slightly tanspartent 
  facet_wrap(~Status)+
  theme_classic()
...
#This is full of problems. For example, the number of bedrooms decreases the price. Also not being forclosed decreases the price. 
...
homes_lm2 = lm(Price~City+Bathrooms+SqFt+Status, data = homes_sub)
homes_lm2
# b. Model summary:
summary(homes_lm2)
```
...
# a. Set Status to class 'factor' #can't use forcats if stored as a character 
...
#shows it's doing it alphabetically 
# d. Reassign reference level of "Status" to "Regular":
#fct_relevel() function in forcats (remember ?fct_relevel)
homes_sub$Status = fct_relevel(homes_sub$Status, "Regular")
#Regular is now first, and other levels are alphabetically behind it
...
plot(homes_lm3)
```
It may look heterosedastic at first, but only a few points are affecting that relationship. 
...
#homes_lm1 if ONLY looking at the AIC value 
#homes_lm3
